```sh
sudo apt -y tall isc-dhcp-server
```

```sh
cat << EOF | sudo tee /etc/dhcp/dhcp-ipxe-op.conf
...
```sh
cat << EOF | sudo tee /etc/dhcp/dhcp-ipxe-hw.conf
...
option ipxe.no-pxedhcp 1; # Disable ProxyDHCP

# Make sure iPXE features support
# If not load a full-featured version.
# See ipxe-option-space.conf for definitions.
if    exists ipxe.http
...
} elsif option arch = 00:07 { filename "ipxe-x64.efi"; # Intel x86-64
...
```sh
cat << EOF | sudo tee /etc/dhcp/dhcp.conf
authoritative;
ddns-update-style none;
use-host-decl-names on;

include "/etc/dhcp/dhcp-ipxe-op.conf";
#include "/etc/dhcp/static.conf";

subnet 192.168.1.0 netmask 255.255.255.0 {
    range 192.168.1.250 192.168.1.252;
    option subnet-mask 255.255.255.0;
    option routers 92.168.1.1;
    ...
    #option lpr-servers 192.168.1.10;
    include "/etc/dhcp/dhcp-ipxe-hw.conf";
}
EOF
```
